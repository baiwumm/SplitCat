...
const exportAsExcel = () => {
    try {
        ...
        const now = new Date();
        const fileName = `SplitCat账单_${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}${now.getDate().toString().padStart(2, '0')}.xlsx`;
    ...
    <div class="flex flex-col gap-6">
        ...
        <div v-else class="flex flex-col gap-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div
                            ...
                            <div class="flex flex-col">
                                <div class="text-xl font-semibold text-gray-800">{{ splitStore.participants.length }}
                                </div>
                        ...
                        <div
                            ...
                            <div class="flex flex-col">
                                <div class="text-xl font-semibold text-gray-800">¥{{ splitStore.totalAmount.toFixed(2)
                                }}</div>
                        ...
                        <div
                            ...
                            <div class="flex flex-col">
                                <div class="text-xl font-semibold text-gray-800">¥{{ (splitStore.totalAmount /
                                    splitStore.participants.length).toFixed(2) }}</div>
            ...
            <NCard class="bg-white rounded-lg shadow-md">
                ...
                <div v-if="transferPlan.length === 0"
        ...
        <!-- 清空数据确认弹窗 -->
        <NModal v-model:show="showClearConfirm" preset="dialog" title="确认清空数据" positive-text="确认清空" negative-text="取消"
            @positive-click="clearAllData" @negative-click="showClearConfirm = false">
        ...
        <NModal v-model:show="showPosterModal" style="width: 100%; max-width: 600px;" preset="card">
            <div class="flex flex-col gap-6">
                ...
                <!-- 海报内容 -->
                <div
                    class="bg-gradient-to-br from-indigo-50 to-blue-50 p-6 rounded-lg border border-indigo-100 shadow-sm">
                    <div class="flex flex-col gap-6">
                        <!-- 海报头部 -->
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="flex flex-col items-center p-3 bg-white rounded-lg shadow-sm">
                                <div class="text-sm text-gray-500">总金额</div>
                                <div class="text-xl font-bold text-indigo-600">¥{{ posterData.totalAmount.toFixed(2) }}
                                </div>
                            </div>
                            <div class="flex flex-col items-center p-3 bg-white rounded-lg shadow-sm">
                                <div class="text-sm text-gray-500">人均消费</div>
                                <div class="text-xl font-bold text-indigo-600">¥{{ posterData.averageAmount.toFixed(2)
                                }}</div>
                            </div>
                            <div class="flex flex-col items-center p-3 bg-white rounded-lg shadow-sm">
                                <div class="text-sm text-gray-500">参与人数</div>
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <NScrollbar>
                                <div class="flex flex-col gap-2">
                                    <div v-for="person in participantSummary" :key="person.id"
                                        class="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                                        <div class="flex items-center gap-3">
                                            <NAvatar size="small" round :style="{
                                                background: person.balance > 0 ? 'linear-gradient(to right, #10b981, #059669)' :
                                                    person.balance < 0 ? 'linear-gradient(to right, #f43f5e, #e11d48)' :
                                        ...
                                        <div :class="[
                                            'font-semibold',
                                            person.balance > 0 ? 'text-emerald-600' :
                                                person.balance < 0 ? 'text-rose-600' : 'text-gray-500'
                                        ]">
                                            {{ person.balance === 0
                                                ? '已结清'
                                                : person.balance > 0
                        ...
                        <div v-if="transferPlan.length > 0" class="flex flex-col gap-3">
                            ...
                            <NScrollbar style="max-height: 200px">
                                <div class="flex flex-col gap-2">
                                    <div v-for="(transfer, index) in transferPlan" :key="index"
                                        class="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                                        <div class="flex items-center gap-2">
                                            <NTag type="error" size="small" round>
                                                {{splitStore.participants.find(p => p.id === transfer.from)?.name ||
                                                    '未知'}}
                                            </NTag>
                                            <NIcon>
